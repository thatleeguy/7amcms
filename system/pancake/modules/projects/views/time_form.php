			...
			<fieldset>
				...
				<div id="invoice-type-block" class="row add-time">
					...
					<div class="row">
						...
						<div class="sel-item">
							<?php echo form_dropdown('task_id', $tasks_select, set_value('task_id', isset($time) ? $time->task_id : '')); ?>
						</div>
					</div>
					<div class="row">
						<label for="note"><?php echo lang('times.label.note'); ?></label>
						<?php echo form_input('note', set_value('note'), 'class="txt"'); ?>
					</div>
					<div class="row">
...
<script type="text/javascript">
	$('#add_time').submit(function() {
		...
		$('#start_time').val(Date.parse($('#start_time').val()).toString('HH:mm'));
		$('#end_time').val(Date.parse($('#end_time').val()).toString('HH:mm'));
		$(this).ajaxSubmit({
		    dataType: 'json',
